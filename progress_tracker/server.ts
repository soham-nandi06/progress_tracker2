...
app.get("/api/stats/codeforces/:username", async (req, res) => {
  ...
  try {
    ...
    if (!infoRes.ok) {
      console.warn(`Codeforces public API returned non-OK status: ${infoRes.status} for handle "${username}". Serving safe fallback profile configurations.`);
      return res.json(fallbackData);
    }
    
    const infoData = await infoRes.json();
    if (infoData.status !== "OK") {
      console.warn(`Codeforces did not return OK status inside payload. Serving fallback profile configurations.`);
      return res.json(fallbackData);
    }
    ...
    try {
      ...
      console.warn("Could not fetch user status timeline, using minimal log array:", statusErr);
    }
    ...
    res.json({
      ...
      submissions: submissions.length > 0 ? submissions : fallbackData.submissions,
    });
  } catch (error: any) {
    console.warn("Gracefully handled Codeforces API exception and returned fallback dataset", error.message);
