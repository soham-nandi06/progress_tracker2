...
export default function App() {
  ...
  // Custom Username-Email/Password Authentication States
    ...
    try {
      if (authMode === "signup") {
        if (!authDisplayName.trim()) {
          setAuthError("A user display name is required.");
          setAuthLoading(false);
          return;
        }
        await signUpWithEmail(authEmail.trim(), authPassword, authDisplayName.trim());
      } else {
  ...
  return (
    <div className="min-h-screen bg-[#050507] text-slate-300 flex flex-col font-sans selection:bg-cyan-500/30 select-none md:select-text" id="app-wrapper">
      ...
      {/* Auth Modal overlay for custom username/password system */}
      {isAuthModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-55 flex items-center justify-center p-4" id="auth-modal-overlay">
          <div className="relative bg-[#0d0e14] border border-slate-800 rounded-2xl w-full max-w-md p-6 overflow-hidden shadow-2xl" id="auth-modal-card">
            
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div>
                <h3 className="text-sm font-bold uppercase tracking-wider font-display text-cyan-400 flex items-center space-x-2">
                  <Cloud className="w-4 h-4 text-cyan-400" />
                  <span>Cloud Storage Account</span>
                </h3>
                <p className="text-[10px] text-slate-400">Sync is activated on Google Firestore</p>
              </div>
              <button 
                onClick={() => setIsAuthModalOpen(false)}
                className="text-slate-400 hover:text-slate-200 transition p-1.5 rounded-lg hover:bg-slate-800/50 cursor-pointer"
                title="Close"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            {/* Selector Tabs */}
            <div className="flex bg-[#050508] border border-slate-800/80 p-1 rounded-lg my-4">
              <button
                type="button"
                onClick={() => {
                  setAuthMode("signin");
                ...
                className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wider rounded-md transition duration-155 ${
                  authMode === "signin" 
            ...
            <form onSubmit={handleEmailAuthSubmit} className="space-y-3.5">
              ...
              {authMode === "signup" && (
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">User Display Name</label>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-[#050508]/90 border border-slate-800 rounded p-2.5 pl-9 text-slate-200 text-xs font-mono focus:outline-none focus:border-cyan-500 transition"
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Email Address</label>
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <input 
                    type="email" 
                    id="auth-email"
                    placeholder="e.g., engineer@trainingcamp.org"
                    className="w-full bg-[#050508]/90 border border-slate-800 rounded p-2.5 pl-9 text-slate-200 text-xs font-mono focus:outline-none focus:border-cyan-500 transition"
                    value={authEmail}
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono block">Secure Password</label>
                <div className="relative">
              ...
              {authError && (
                <div className="bg-rose-500/10 border border-rose-500/25 p-3 rounded text-[11px] text-rose-400 font-mono flex items-start space-x-2">
              ...
              <button
                ...
                className="w-full bg-cyan-500 hover:bg-cyan-400 disabled:opacity-50 text-slate-950 font-bold uppercase tracking-wider py-2.5 rounded text-xs transition duration-200 flex items-center justify-center space-x-2 cursor-pointer shadow-[0_0_12px_rgba(34,211,238,0.15)]"
              >
                {authLoading ? (
            ...
            <div className="relative my-4 flex items-center justify-center">
              ...
              <span className="relative bg-[#0d0e14] px-3.5 text-[9px] uppercase tracking-widest font-mono text-slate-500 font-bold">OR</span>
            </div>

            {/* Google Authentication popup anchor */}
            <button
              onClick={() => {
                setIsAuthModalOpen(false);
                handleGoogleSignIn();
              }}
              className="w-full bg-slate-900 hover:bg-slate-850 border border-slate-800 py-2.5 rounded text-xs font-semibold text-slate-300 transition duration-155 flex items-center justify-center space-x-2 cursor-pointer"
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24">
                <path
                  fill="currentColor"
                  d="M12.24 10.285V14.4h6.887c-.648 2.41-2.519 4.114-5.136 4.114A5.932 5.932 0 0 1 8 12.571a5.932 5.932 0 0 1 5.99-5.943c1.558 0 2.978.583 4.043 1.537l3.223-3.2A11.026 11.026 0 0 0 13.99 1.714C7.886 1.714 3 6.571 3 12.571s4.886 10.857 10.99 10.857c6.31 0 10.96-4.385 10.96-10.857 0-.74-.082-1.428-.21-2.286H12.24Z"
                />
              </svg>
