...
import { ChevronDown, ChevronUp, CheckSquare, Square, Edit2, Check, HelpCircle, AlertCircle, Plus, BookOpen, Layers, RefreshCw, LayoutGrid, List } from "lucide-react";
...
export default function Schedule({ days, onUpdateDay, onAddDay, startDate, onUpdateStartDate, onResetProgress }: ScheduleProps) {
  const [viewMode, setViewMode] = useState<"notion" | "classic">("notion");
  ...
  return (
    <div className="space-y-6" id="schedule-container">
      ...
      {/* Top Level Layout Selector Toggle (Notion Workspace vs Classic Checklist) */}
      <div className="flex bg-[#08080c] border border-slate-800/85 p-1.5 rounded-xl text-xs font-mono font-bold uppercase tracking-wider gap-1 max-w-sm mb-4">
        <button 
          onClick={() => setViewMode("notion")}
          className={`flex items-center justify-center space-x-1.5 px-3 py-2 rounded-lg flex-1 cursor-pointer transition duration-150 ${
            viewMode === "notion" 
      ...
      {/* Dynamic Schedule Configurations & Reset panel */}
      <div className="bg-[#0c0d12] border border-slate-800/80 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4 shadow-md" id="schedule-config-hud">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full md:w-auto">
          {/* Calendar start date option */}
          <div className="flex flex-col gap-1.5 w-full sm:w-auto">
            <span className="text-[10px] uppercase tracking-wider font-mono text-slate-500 font-bold">Training Start Date</span>
            <div className="flex items-center gap-2">
              <input 
                type="date" 
                value={startDate || ""}
                onChange={(e) => onUpdateStartDate(e.target.value)}
                className="bg-[#14151f] border border-slate-805 text-slate-200 text-xs rounded px-3 py-1.5 focus:outline-none focus:border-cyan-500/50 hover:bg-[#171824] transition w-full sm:w-44 font-mono uppercase"
              />
              {startDate && (
                <button 
                  onClick={() => onUpdateStartDate("")}
                  className="text-[10px] text-slate-500 hover:text-rose-450 font-mono underline uppercase"
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-end sm:justify-start">
          {/* Reset progress action option */}
          <button 
            onClick={onResetProgress}
            className="bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 hover:border-rose-500/35 text-rose-400 px-4 py-2 rounded text-xs tracking-wider uppercase font-bold transition flex items-center space-x-1.5 cursor-pointer"
            title="Reset all days & checklists back to initial seed"
            ...
            >
              ...
              >
                <div className="flex items-center space-x-3 w-full sm:w-auto">
                  <div className={`p-1.5 rounded-lg font-mono font-bold text-xs h-11 w-11 flex flex-col items-center justify-center shrink-0 border leading-tight ${
                    day.completed 
                      ? "bg-cyan-500/10 border-cyan-500/40 text-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.15)]" 
                      : day.isBacklog 
                        ? "bg-amber-500/10 border-amber-500/40 text-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.1)]" 
                        : "bg-[#161720] border-slate-800 text-slate-450"
                  }`}>
                    <span className="text-xs">D{day.dayNumber}</span>
                    {startDate && (
                      <span className="text-[8px] font-semibold text-slate-400 mt-0.5 uppercase tracking-tighter opacity-80 shrink-0">
                        {(() => {
                  ...
                  <div className="space-y-0.5 truncate flex-1">
                    <div className="flex items-center flex-wrap gap-2">
                      ...
                      {startDate && (
                        <span className="text-[10px] text-cyan-400 font-mono border border-cyan-500/20 bg-cyan-500/5 px-2 py-0.5 rounded select-none shadow-[0_0_8px_rgba(34,211,238,0.05)]">
                          {(() => {
                            const start = new Date(startDate);
                            if (!isNaN(start.getTime())) {
                              start.setDate(start.getDate() + (day.dayNumber - 1));
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3.5" id={`blocks-list-day-${day.dayNumber}`}>
                          ...
                          <div 
                            ...
                            className={`p-4 rounded-xl border cursor-pointer transition duration-200 select-none flex flex-col justify-between min-h-[112px] h-auto gap-2 ${
                          ...
                          >
                            ...
                            <p className="text-xs font-bold leading-relaxed text-slate-200 break-words">
                          ...
                          >
                            ...
                            <span className="text-[9px] text-amber-550 font-mono font-bold uppercase tracking-widest">Codeforces</span>
                          </div>
                          ...
                          >
                            ...
                            <span className="text-[9px] text-pink-400 font-mono font-bold uppercase tracking-widest">ML Theory</span>
                          </div>
                          ...
                          >
                            <div className="flex justify-between items-start">
                              ...
                              {day.block5_completed ? <CheckSquare className="w-4 h-4 text-purple-400 shrink-0" /> : <Square className="w-4 h-4 shrink-0" />}
                            </div>
                            <p className="text-xs font-bold leading-relaxed text-slate-200 break-words">
                              {day.block5_ML_Project}
                            </p>
                            <span className="text-[9px] text-purple-400 font-mono font-bold uppercase tracking-widest">ML Practice</span>
                          </div>
                        </div>
                      </div>
